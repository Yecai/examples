  ...
  public void run(boolean inClearJobs, boolean inScheduleJobs) throws Exception {
    ...
    if (inScheduleJobs) {
      ...
      SimpleTrigger trigger = newTrigger().withIdentity("triger_" + count, schedId)
          ...
          .withSchedule(simpleSchedule().withRepeatCount(200).withIntervalInSeconds(5)).build();
...
//      count++;
//
//      job = newJob(SimpleRecoveryStatefulJob.class).withIdentity("job_" + count, schedId) // put triggers in group named after
